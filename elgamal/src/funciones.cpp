...
funcions_math::funcions_math(){}

ZZ funcions_math::getMod(ZZ d, ZZ a){
    ZZ q,r;
    q = d/a;
    r = d-(q*a);
    if(r<0){
...
ZZ funcions_math::algEuclides(ZZ a, ZZ b){
    ZZ m;
    m = getMod(a,b);
    while(m!=0){
        a=b;
...
ZZ funcions_math::algEuclidesExt(ZZ a, ZZ b){
...
ZZ funcions_math::Inversa(ZZ a,ZZ l){
...
int funcions_math::getModInt(int d,int a){
    int q = d/a;
    int r = d-(q*a);
    if(r<0){
        r+=a;
...
ZZ funcions_math::genetareRandom(int sizeSeed,int sizeNumber,int numPar,int loops){
    vector<bool> seed;
    bool control = 1;
...
ZZ funcions_math::MulMod(ZZ a, ZZ b, ZZ mod){
    ZZ x,y,c;
    c=2;
    x = 0;
    y = getMod(a,mod);
...
bool funcions_math::testMiller(ZZ pseudoPrime,int iteration){
    ZZ s,a,temp,mod,c;
    c=2;
    if (pseudoPrime < c)
    {
...
ZZ funcions_math::modularExpo(ZZ a, ZZ b, ZZ m)
{
	ZZ ans, c;
	ans = 1;
	c = 2;
...
bool funcions_math::fermatTest(int numTests,ZZ pseudoPrime){
    funcions_math m;
    ZZ a,c;
    c=1;
    if(pseudoPrime==2){
...
ZZ funcions_math::chineseReminder(vector<ZZ> ai,vector<ZZ> pi,ZZ n,ZZ k){
    funcions_math m;
...
vector<ZZ>funcions_math::RndNumber(){
